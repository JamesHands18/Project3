...
// url = `http://127.0.0.1:5000/api/base`
...
function init() {
  ...
  d3.json(`http://127.0.0.1:5000/api/base`).then(function(data) {
    ...
    var layout = {
      title: "Top 5 Asteroids by Estimated Diameter (km)",
      height: 600,
      margin: {
        l: 100,
        r: 100,
...
function updatePlotly() {
  ...
  console.log("selOption",selOptionVal);
  ...
  if (selSortingVal > 0) {
    ...
    console.log("descending",sliceBy);
  ...
  d3.json(`/api/base`).then(function(data) {
    ...
    if (descending === true) {
      if (selOptionVal === 'estimated_diameter') {
        ...
        var text_title = `Top ${sliceBy} Asteroids By Diameter (km)`;
        console.log(x)
      }
        ...
        var y = reversedData.map(asteroid => asteroid.name);
        var text = reversedData.map(asteroid => asteroid.name);
        var text_title = `Top ${sliceBy} Asteroids By Distance (Lunar units)`;
      }
      
    }
    else {
      if (selOptionVal === 'estimated_diameter') {
        var sortedasteroids = data.sort((a, b) => a.estimated_diameter - b.estimated_diameter);
        var slicedData = sortedasteroids.slice(0, sliceBy);
        var reversedData = slicedData;
        var x = reversedData.map(asteroid => asteroid.estimated_diameter);
        var y = reversedData.map(asteroid => asteroid.name);
        var text = reversedData.map(asteroid => asteroid.name);
        var text_title = `Bottom ${sliceBy} Asteroids By Diameter (km)`;
      }
      else if (selOptionVal === 'distance') {
        var sortedasteroids = data.sort((a, b) => a.distance - b.distance);
        var slicedData = sortedasteroids.slice(0, sliceBy);
        var reversedData = slicedData;
        var x = reversedData.map(asteroid => asteroid.distance);
        var y = reversedData.map(asteroid => asteroid.name);
        var text = reversedData.map(asteroid => asteroid.name);
        var text_title = `Bottom ${sliceBy} Asteroids By Distance (Lunar units)`;
